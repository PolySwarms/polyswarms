import "dotenv/config";
import * as anchor from "@coral-xyz/anchor";
import { Keypair, SystemProgram, PublicKey } from "@solana/web3.js";
import fs from "fs";

const [market] = process.argv.slice(2);
if (!market) { console.log("Usage: yarn claim <MARKET_PUBKEY>"); process.exit(1); }

const RPC = process.env.RPC_URL!;
const PROGRAM_ID = new PublicKey(process.env.PROGRAM_ID!);
const WALLET_PATH = (process.env.WALLET || `${process.env.HOME}/.config/solana/id.json`) as string;

const kp = Keypair.fromSecretKey(new Uint8Array(JSON.parse(fs.readFileSync(WALLET_PATH, "utf8"))));
const provider = new anchor.AnchorProvider(new anchor.web3.Connection(RPC, "confirmed"), new anchor.Wallet(kp), {});
anchor.setProvider(provider);

const idl = await anchor.Program.fetchIdl(PROGRAM_ID, provider);
const program = new anchor.Program(idl!, PROGRAM_ID, provider);

const marketPk = new PublicKey(market);
const [yesPool] = PublicKey.findProgramAddressSync([Buffer.from("pool"), marketPk.toBuffer(), Buffer.from("YES")], PROGRAM_ID);
const [noPool]  = PublicKey.findProgramAddressSync([Buffer.from("pool"), marketPk.toBuffer(), Buffer.from("NO")], PROGRAM_ID);

// pick which bet PDA: winner side seed—program checks outcome internally
const out = await program.account.market.fetch(marketPk) as any;
const outcome = out.outcome.yes ? "YES" : "NO";
const [bet] = PublicKey.findProgramAddressSync([Buffer.from("bet"), marketPk.toBuffer(), kp.publicKey.toBuffer(), Buffer.from(outcome)], PROGRAM_ID);

await program.methods.claim().accounts({
  user: kp.publicKey,
  market: marketPk,
  yesPool,
  noPool,
  bet,
  systemProgram: SystemProgram.programId
}).rpc();

console.log("Claimed payout");
